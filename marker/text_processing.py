...
def highlight(raw_text,
    ...
    if removeStopWords:
        ...
        clean_text = [(i, w) for (i, w) in preprocess_text if re.search(
            '[a-zA-Z0-9]', w) and w not in stop_words]
    else:
        clean_text = [(i, w) for (i, w) in preprocess_text if re.search(
            '[a-zA-Z0-9]', w)]  # and w not in stop_words]
    ...
    for (position, word) in enumerate(clean_text):
        ...
        for i in range(len(boldness_scores)):
            boldness_total_scores[clean_text[i + offset][0]
                                  ] += boldness_scores[i] * len(boldness_scores)
    ...
    if normalization:
        for i in range(len(boldness_total_scores)):
            boldness_total_scores[i] = boldness_total_scores[i] / \
                normalization * (1 - boldness_baseline) + boldness_baseline
    else:
        boldness_total_scores = [boldness_baseline] * \
            len(boldness_total_scores)
    # Calculate boldness for each character
    final_boldness = [0] * len(raw_text)
    cnt = 0
    for i in range(len(words_lenghts)):
        length = words_lenghts[i]
        final_boldness[cnt:cnt +
                       length] = model_boldness(length, boldness_total_scores[i])
        cnt += length + 1
